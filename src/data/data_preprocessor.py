...
class DataPreprocessor:
    ...
    def get_train_generator() -> DirectoryIterator:
        return DataPreprocessor._get_generator(DataSet.TRAIN)

    @staticmethod
    def get_validation_generator() -> DirectoryIterator:
        return DataPreprocessor._get_generator(DataSet.VALIDATION)
    ...
    def _get_generator(data_set: DataSet) -> DirectoryIterator:
        return DataPreprocessor._data_generator.flow_from_directory(
            DataPreprocessor._get_path(data_set),
            target_size=(Config.PICTURE_SIZE, Config.PICTURE_SIZE),
    ...
    def _get_path(
        ...
        return '../' + Config.DATA_DIRECTORY + data_set.value + "/" \
    ...
    def get_all_pictures_metadata(
        ...
        if emotion == Emotion.UNSPECIFIED:
            for emotion in Emotion:
                DataPreprocessor._add_pictures_metadata_to_set(list_, data_set, emotion)
        else:
            DataPreprocessor._add_pictures_metadata_to_set(list_, data_set, emotion)
        return list_
    ...
    def _add_pictures_metadata_to_set(list_: list, data_set: DataSet, emotion: Emotion) -> None:
        path = DataPreprocessor._get_path(data_set, emotion)
        if isdir(path):
